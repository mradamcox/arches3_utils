...
def make_files(data, out_dir, concept_scheme_name):
    ...
    with open(authdoc_path, "wb") as authdoc:
        ...
        for num,label in enumerate(concepts):
            ...
            row = [con,label,"",authdoc_name.replace("_blank",""),"Index",concept_scheme_name]
            writer.writerow(row)
    ...
    with open(values_path, "wb") as values:
        ...
        for num,label in enumerate(concepts):
            con = conceptid+":"+str(num+1)
            writer.writerow([con,str(num+1),"sortorder",concept_scheme_name])

while True:
    data = get_info()
    make_files(data,auth_dir,concept_scheme_name)
    print "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n"
